...
function timeShow(){
    ...
    if(date !== ""){
        document.getElementById("timeSelector").classList.remove("destroy")
    }
}
...
function checkRadio(val){
    ...
    let itm = document.getElementById("time"+val);
    let itm2 = document.getElementById("timeEnd"+val);

    let valu = "0";
    let valu2 = "0";

    if(itm.checked){
        document.getElementById("timeSelector2").classList.remove("destroy");
    }
        ...
        for(let i =0; i<12; i++){
            if(document.getElementById("time"+i)) {
                if (document.getElementById("time" + i).checked) {
                    valu = document.getElementById("time" + i).value;
                }
            }
        }
        valu2 = itm2.value;
    ...
    if(valu !== "0" && valu2 !=="0"){
        document.getElementById("duration").classList.remove("destroy");

        document.getElementById("durationInput").value = (parseInt(valu2) - parseInt(valu)).toString()+"hours";
    }
...
function switchPage(num){
    ...
    for(let i = 0; i<pages.length; i++){
        if(pages[i] !== num){
            not_num.push(pages[i]);
        }
    }

    for(let i =0; i<not_num.length; i++){
        if (document.getElementById("pg"+not_num[i]).classList.contains("points-page")){
            document.getElementById("body"+not_num[i]).classList.add("destroy");
            document.getElementById("pg"+not_num[i]).classList.remove("active-warning");
        }else{
            document.getElementById("body"+not_num[i]).classList.add("destroy");
            document.getElementById("pg"+not_num[i]).classList.remove("active");
        }
    }

    if (document.getElementById("pg"+num).classList.contains("points-page")){
        document.getElementById("body"+num).classList.remove("destroy");
        document.getElementById("pg"+num).classList.add("active-warning");
    }else{
        document.getElementById("body"+num).classList.remove("destroy");
        document.getElementById("pg"+num).classList.add("active");
    }
